...
Статус: утверждённая техническая спецификация
...
| Primary runtime model | configured live provider profile after exact-ID/capability/latency smoke |
...
| Optional comparator route | disabled by default, separate run opt-in/caps |
...
| Specification root | `communication-factory-spec/` (ASCII) |
| Implementation completion gate | `make verify-implementation` |
...
- primary обычных/live cases: явно выбранный и проверенный provider profile внутри Ouroboros;
- comparator route: выключен по умолчанию; только отдельный явный golden/comparator run с opt-in и token cap, если latency/SLO позволяют;
...
Каждый paid smoke/pilot/comparator/evaluation run требует положительные `EVAL_MAX_TOKENS` и `EVAL_MAX_COST_USD`. Это project-local run caps, а не утверждение о provider-account balance. Для comparator-run дополнительно обязательны отдельный явный opt-in, новый `COMPARATOR_RUN_ID` и отдельный cap из operator profile.
...
После frozen live run система формирует минимум шесть immutable representative review packets без предзаполненных оценок. Затем человек полностью читает их и оценивает 1–5:
...
На VPS единственный ожидаемый owner source — `/home/dmitry/secrets/communication-factory/OPENAI_API_KEY.txt` вне Git checkout, mode `0600`. Любая копия ключа внутри VPS repository является security violation даже при наличии `.gitignore`. Автоматизированная проверка может установить только presence/form без значения; перемещать, удалять или ротировать owner source без approval нельзя. Обнаружение ключа в Git/history/artifact — stop-the-line и rotation.
...
Этот ключ не является разрешением передавать его инструментам разработки, запускать прямые SDK experiments или обходить Ouroboros/live-run guards. Модель инженерного control plane не относится к runtime usage этого проекта.
...
DoD разделён на машинно-достижимый Implementation DoD и человеческий Submission DoD. Первый должен быть полностью закрыт автоматизированными проверками, а второй заполняется только командой.

### 23.1. Implementation DoD — автоматизированная проверка
...
- [ ] Every paid run stays within its explicit token/dollar caps and project-observed planning allowance; account-wide remaining quota is reported as unknown. The optional comparator ran only with explicit opt-in, if at all.
...
# Optional comparator route is disabled by default
COMPARATOR_RUN_ID=
...
OUROBOROS_ACCEPTANCE_MAX_IMPROVEMENT_PASSES=0
OUROBOROS_POST_TASK_EVOLUTION=false
OUROBOROS_MODEL=<configured runtime route>
OUROBOROS_MODEL_LIGHT=<configured lightweight runtime route>
...
`OPERATOR_LIMITS_FILE` — read-only mount внешнего owner-файла; его числа не являются schema constants. `TOTAL_BUDGET` и `OUROBOROS_PER_TASK_COST_USD` используют встроенные controls pinned runtime и уточняются после pilots. `EVAL_MAX_TOKENS=0`, `EVAL_CONCURRENCY=1`, выключенный comparator route, `OPENROUTER_ENABLED=false` и нулевые OpenRouter caps являются fail-closed defaults. Live runner требует явных положительных token/dollar overrides, но не заявляет знание account-wide remainder.
...
assert budget exhaustion cannot retry or switch model/provider; comparator route and OpenRouter default off

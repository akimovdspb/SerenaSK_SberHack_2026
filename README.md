...
Внутренние GitHub-ветки и Railway-стенд остаются рабочими контурами команды; для оценки кода
используется публичная ссылка выше.
...
Текущий release status: `WAITING_FOR_OPERATOR` — финальная инженерная UI-полировка прошла полный
providerless-контур: `make verify-core` 21/21,
36 браузерных сценариев пройдены, два штатно пропущены в неприменимом профиле; security, Ruff,
mypy и frontend build зелёные. Публичный репозиторий и презентация подготовлены для оценки;
ссылка на озвученное демо-видео будет добавлена после загрузки.

Актуальная конкурсная кодовая линия включает обычный мастер новой кампании, live-генерацию
СМС/e-mail через Railway-профиль GLM-5.2, один контролируемый transport-retry, замечание редактора,
неизменяемую версию 2 с diff, предложение ограниченного правила и его применение к следующей
подходящей кампании после решения человека. Hosted-путь фактически пройден; подробности и точные
границы проверки записаны в [STATUS.md](STATUS.md) и [Railway runbook](docs/RAILWAY.md).

Практический конкурсный порог достигнут независимо от нового демонстрационного authoring-пути:
10 живых кейсов имеют сохранённые фактические выходы, метрики использования и QA 100/100.
Полный каталог B01–B15 и честный технический протокол 13/15 сохранены без сокращения. Новые
демо-кампании не добавляются к этой десятке и не меняют исторические выходы задним числом.

Финальная полировка интерфейса долгих операций реализована и прошла providerless браузерную
квалификацию, отдельные security-проверки и полный core 21/21 без обращений к провайдеру.
Презентация и очищенный публичный репозиторий подготовлены; для конкурсной сдачи остаётся записать
и опубликовать озвученное демо-видео длительностью менее трёх минут, добавить ссылку в материалы и
передать их организаторам. Чат и голос остаются выключенным P1; весь контур расширений P1/P2 в
конкурсный релиз не входит.
